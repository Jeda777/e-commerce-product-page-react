    ...
    return (
        <header className='w-full max-w-[1100px] p-6 flex justify-between relative border-b border-very-dark-blue'>
            ...
            <div className='flex items-center gap-4 lg:gap-14'>
                <nav className={` ${menuOpen ? 'left-0' : '-left-[120vw]'} bg-black w-screen bg-opacity-75 absolute h-screen z-20 top-0 
                ...
                <img src={menuOpen ? IconClose : IconMenu} onClick={() => setMenuOpen((prev) => !prev)} alt='menu'
                ...
                <nav className='hidden lg:flex text-dark-grayish-blue gap-8'>
                    <a className='desktop-nav-link' href="#">Collections</a>
                    <a className='desktop-nav-link' href="#">Men</a>
                    <a className='desktop-nav-link' href="#">Women</a>
                    <a className='desktop-nav-link' href="#">About</a>
                    <a className='desktop-nav-link' href="#">Contact</a>
                </nav>
            </div>
            ...
            <div className='flex gap-5 items-center'>
                <div>
                    <div className='relative'>
                        ...
                        <p className={`${cartItems.length !== 0 ? '' : 'hidden'} text-white bg-primary px-2 rounded-md absolute
                        top-[-7px] right-[-7px] text-xs`}>
                            {cartItems.length}
                        </p>
                    </div>
                    <div className={`bg-white absolute w-[95%] right-[2.5%] ${cartOpen ? 'top-20' : 'top-[-5000%]'}  rounded-xl
                    transition-all max-w-[360px] shadow-xl z-10`}>
                        ...
                        <div className='p-6 flex flex-col gap-6'>
                            <ul className='flex flex-col gap-4'>
                                <li className={cartItems.length === 0 ? '' : 'hidden'}><p className='text-center'>
                                    Your cart is empty.</p></li>
                                    ...
                                    <li key={index} className='flex items-center gap-2'>
                                        ...
                                        <div className='text-sm w-full'>
                                            ...
                                            <div className='flex gap-4 items-center'>
                                                <p className='text-dark-grayish-blue'>${item.price} x {item.count}</p>
                            ...
                            <button className='w-full text-center bg-primary py-5 rounded-xl text-white
                            hover:opacity-50 transition-all'>Checkout</button>
                        </div>
                    </div>
                </div>
                <img src={Avatar} alt="Avatar" className='w-6 h-6 lg:w-12 lg:h-12 rounded-full cursor-pointer hover:outline-2 hover:outline-primary
