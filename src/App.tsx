            ...
            {ImageProductsThumbnails.map((item, index: number) => (
              <img
                ...
                className={`rounded-lg cursor-pointer hover:opacity-50 transition-all ${
                  imgIndex === index ? 'opacity-50 outline outline-2 outline-primary' : ''
                }`}
        ...
        <div className='p-6 flex flex-col gap-4 lg:p-0 lg:basis-1/2'>
          ...
          <div className='flex flex-col gap-4 lg:flex-row'>
            <div className='relative lg:basis-1/3'>
              <button
                className='absolute translate-y-[-50%] top-[50%] left-6 hover:opacity-50'
              ...
              <input
                ...
                min={0}
                step={1}
                className='w-full text-center bg-light-grayish-blue p-6
              rounded-2xl font-bold'
                onChange={(e) => (e.target.value !== '' ? setCountToAdd(Number(e.target.value)) : setCountToAdd(0))}
              />
              <button
                className='absolute translate-y-[-50%] top-[50%] right-6 hover:opacity-50'
            ...
            <button
              className='flex gap-4 text-center bg-primary justify-center items-center text-white font-bold p-5
            rounded-2xl hover:opacity-50 lg:basis-2/3'
              onClick={() =>
                countToAdd > 0
                  ? setCartItems([...cartItems, { name: 'Fall Limited Edition Sneakers', price: 125, count: countToAdd }])
                  : null
              }
            >
              <img src={IconCartWhite} alt='cart' /> Add to cart
            </button>
          </div>
        </div>

        <div
          className={`hidden ${lightboxOpen ? 'lg:flex' : ''} justify-center items-center w-full h-full bg-black bg-opacity-75
        absolute left-0 top-0`}
        >
          <div className='flex flex-col gap-8 items-center max-w-[800px] max-h-[80vh]'>
            <button className='self-end' onClick={() => setLightboxOpen(false)}>
              <img
                src={IconCloseWhite}
            ...
            <div className='relative'>
              <button
                className='w-12 h-12 bg-white absolute flex justify-center items-center rounded-full translate-y-[-50%]
              top-[50%] left-0 translate-x-[-50%]'
                onClick={() => decreaseImgIndex()}
                onMouseEnter={(e) => {
                  const el = e.currentTarget.children[0] as HTMLImageElement
                  el.src = IconPreviousOrange
                }}
                onMouseLeave={(e) => {
                  const el = e.currentTarget.children[0] as HTMLImageElement
                  el.src = IconPrevious
              ...
              <button
                className='w-12 h-12 bg-white absolute flex justify-center items-center rounded-full translate-y-[-50%]
              top-[50%] right-0 translate-x-[50%]'
                onClick={() => increaseImgIndex()}
                onMouseEnter={(e) => {
                  const el = e.currentTarget.children[0] as HTMLImageElement
                  el.src = IconNextOrange
                }}
                onMouseLeave={(e) => {
                  const el = e.currentTarget.children[0] as HTMLImageElement
                  el.src = IconNext
